...
* **Persistência de Dados via Volume Nomeado (`oracle-clyvovet-data`):** Para assegurar a conformidade e a durabilidade exigidas em auditorias de prontuários clínicos, o container do Oracle Database monta os arquivos de tabelas (`/opt/oracle/oradata`) diretamente no volume persistente do host Linux. Isso garante que reinicializações ou atualizações críticas do container do banco de dados não causem perda dos dados das transações de CRUD efetuadas na aplicação.
